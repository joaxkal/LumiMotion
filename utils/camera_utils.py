...
def loadCam(args, id, cam_info, resolution_scale):
    orig_w, orig_h = cam_info.image_train_light.size
    ...
    resized_image_rgb_train_light = PILtoTorch(cam_info.image_train_light, resolution)
    gt_image_train_light = resized_image_rgb_train_light[:3, ...]

    loaded_mask = None

    if resized_image_rgb_train_light.shape[0] == 4:
        loaded_mask = resized_image_rgb_train_light[3:4, ...]

    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY,
                  gt_alpha_mask=loaded_mask,
                  image_train_light=gt_image_train_light, 
                  image_name_train_light=cam_info.image_name_train_light, 
                  image_path_train_light = cam_info.image_path_train_light,
                  uid=id,
                  data_device=args.data_device if not args.load2gpu_on_the_fly else 'cpu', fid=cam_info.fid,
                  depth=cam_info.depth, proj_matrix= cam_info.proj_matrix)
...
def camera_to_JSON(id, camera: Camera):
    ...
    camera_entry = {
        'id': id,
        'img_name_train_light': camera.image_name_train_light,
        'width': camera.width,
