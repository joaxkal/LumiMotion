...
def find_static_dataset(source_path):
    matches = glob.glob(f"{source_path}_statictimestep*")
    if not matches:
        raise FileNotFoundError(f"No static dataset found for: {source_path}")
    print("Found static dataset: ", matches[0])
    return matches[0]
