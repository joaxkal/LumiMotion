...
def render_set(dataset: ModelParams, pipeline: PipelineParams, load_iter):
    with torch.no_grad():
        ...
        render_path_parent = os.path.join(dataset.model_path, "scale_albedo_dynamic_preview",
        ...
        for render_idx, view in enumerate(cameras[:]):
            ...
            torchvision.utils.save_image(rendering.clamp(0.0,1.0), os.path.join(render_path, 'albedo_train_dynamic_linear{}'.format(render_idx) + ".png"))
            ...
            torchvision.utils.save_image(albedo_gt.clamp(0.0, 1.0), os.path.join(render_path, f'albedo_gt_dynamic_srgb{render_idx}.png'))
            albedo_gt = srgb_to_rgb(albedo_gt)
            torchvision.utils.save_image(albedo_gt.clamp(0.0, 1.0), os.path.join(render_path, f'albedo_gt_dynamic_linear{render_idx}.png'))
        ...
        with open(os.path.join(args.model_path, "albedo_scale_linear_dynamic.json"), "w") as f:
