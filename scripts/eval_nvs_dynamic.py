...
def render_set(dataset: ModelParams, pipeline: PipelineParams, opt: OptimizationParams, load_iter):
    with torch.no_grad():
        ...
        dynamic_relight_folder = os.path.join(dataset.source_path, dataset.train_light_folder)

        cameras = scene.getTestCameras()
        assert len(cameras) > 0

        render_path_parent = os.path.join(dataset.model_path, "eval_nvs_dynamic",
        ...
        for render_idx, view in enumerate(tqdm(cameras[:], desc="Eval dynamic nvs")):
        ...
        with open(os.path.join(args.model_path, "results_nvs_dynamic.json"), "w") as f:
            json.dump(results_dict, f, indent=4)
        print("Results saved to", os.path.join(args.model_path, "results_nvs_dynamic.json"))
...
if __name__ == "__main__":
    ...
    render_set(model.extract(args), pipeline.extract(args), opt.extract(args), args.load_iter)