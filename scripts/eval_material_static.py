...
def render_set(dataset: ModelParams, pipeline: PipelineParams, load_iter, static_dataset):
    with torch.no_grad():
        ...
        render_path_parent = os.path.join(dataset.model_path, "eval_material_static",
        ...
        for render_idx, view in enumerate(cameras[:]):
            ...
            torchvision.utils.save_image(rendering.clamp(0.0,1.0), os.path.join(render_path, 'albedo_train_static{}'.format(render_idx) + ".png"))
            torchvision.utils.save_image(rendered_roughness, os.path.join(render_path, f"roughness_train_static{render_idx}.png"))
            ...
            torchvision.utils.save_image(albedo_gt.clamp(0.0, 1.0), os.path.join(render_path, f'albedo_gt_static{render_idx}.png'))
            ...
            #mse for roughness
            roughness_gt_name = f"r_{render_name[-4:]}.png"  # extract last 4 digits
            roughness_gt_path = os.path.join(static_dataset, "roughness", roughness_gt_name)
            roughness_gt = read_image(roughness_gt_path).float()[:3]
            roughness_gt = F.interpolate(roughness_gt.unsqueeze(0), size=target_size, mode='bilinear', align_corners=False).squeeze(0).cuda()
            roughness_gt /= 255.0
            torchvision.utils.save_image(roughness_gt, os.path.join(render_path, f"roughness_gt_static{render_idx}.png"))

            roughness_masked = rendered_roughness * mask
            roughness_gt_masked = roughness_gt * mask

            mse_roughness += F.mse_loss(roughness_masked, roughness_gt_masked).double().item()
        ...
        print("\nEvaluating AVG: PSNR_ALBEDO {: .2f} SSIM_ALBEDO {: .3f} LPIPS_ALBEDO {: .3f} MSE_ROUGHNESS {:.3f}".format(psnr_albedo, ssim_albedo, lpips_albedo, mse_roughness))
        with open(os.path.join(args.model_path, "results_material_static.json"), "w") as f:
            json.dump(results_dict, f, indent=4)
        print("Results saved to", os.path.join(args.model_path, "results_material_static.json"))
