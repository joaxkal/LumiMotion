...
def render_set(dataset: ModelParams, pipeline: PipelineParams, load_iter, static_dataset):
    with torch.no_grad():
        ...
        render_path_parent = os.path.join(dataset.model_path, "eval_relight_static",
        ...
        for render_idx, view in enumerate(tqdm(cameras[:], desc="Eval static relight")):
            ...
            # render_pkg_relight["light_indirect"] = render_pkg_relight["light_indirect"] * mask + (1 - mask) * bg
            # torchvision.utils.save_image(render_pkg_relight["light_indirect"].clamp(0.0, 1.0), os.path.join(render_path, f'render_indirect{render_idx}.png'))
        ...
        with open(os.path.join(args.model_path, "results_relight_static.json"), "w") as f:
            json.dump(results_dict, f, indent=4)
        print("Results saved to", os.path.join(args.model_path, "results_relight_static.json"))
