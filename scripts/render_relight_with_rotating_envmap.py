...
def render_set(dataset: ModelParams, pipeline: PipelineParams, load_iter, colmap_convention):
    ...
    with torch.no_grad():
        ...
        gaussian_indices = torch.arange(N, device=gaussians.get_albedo.device).view(N, 1)
        ...
        render_path_parent = os.path.join(dataset.model_path, "render_relight_with_rotating_light", "ours_{}".format(scene.loaded_iter))
        ...
        for render_idx, view in enumerate(cameras[:]):
            ...
            selected_timesteps = [first, last] #or [first] or timesteps...
            ...
            for interp_fid, timestep_idx in tqdm(selected_timesteps, desc="generate image for one timestep"):
                ...
                # We manually design 32x16 HDR / 64x32 HDR. Light one pixel to create point light. 
                # Changing lit pixel coordinates creates an effect of rotating light.
                # For example: changing coord i from 32 to 42 rotates envmap horizontally.

                #========== default for synthetic scenes: 32x16
                # W, H = 32, 16
                # coords = [(i, j) for j in range(4,5, 1) for i in range(25,26, 1)]  

                #========== default for enerf scenes: 64x32
                W, H = 64, 32
                coords = [(i, j) for j in range(12, 14, 2) for i in range(1, 63, 2)]

                for final_i, final_j in tqdm(coords, desc="Pixel traversal (envmap rotation)"):
                    
                    # light all pixels with small value
                    hdr_np = np.zeros((H, W, 3), dtype=np.float32)+0.2

                    # light one pixel with desired value - white, more purple...

                    ## white light
                    # hdr_np[final_j, final_i] = 100 #500  # Set one bright pixel (note: j = row (Y), i = col (X))
                    
                    ## more purple light
                    # hdr_np[final_j, final_i,0] = 90 # Set one bright pixel (note: j = row (Y), i = col (X))
                    # hdr_np[final_j, final_i,1] = 80 # Set one bright pixel (note: j = row (Y), i = col (X))
                    # hdr_np[final_j, final_i,2] = 100 # Set one bright pixel (note: j = row (Y), i = col (X))

                    ## more green light
                    # hdr_np[final_j, final_i,0] = 90  # Set one bright pixel (note: j = row (Y), i = col (X))
                    # hdr_np[final_j, final_i,1] = 110  # Set one bright pixel (note: j = row (Y), i = col (X))
                    # hdr_np[final_j, final_i,2] = 80  # Set one bright pixel (note: j = row (Y), i = col (X))

                    ## more yellow light
                    hdr_np[final_j, final_i,0] = 110  # Set one bright pixel (note: j = row (Y), i = col (X))
                    hdr_np[final_j, final_i,1] = 110  # Set one bright pixel (note: j = row (Y), i = col (X))
                    hdr_np[final_j, final_i,2] = 90  # Set one bright pixel (note: j = row (Y), i = col (X))

                    # blurred = np.zeros_like(hdr_np)
                    # for c in range(3):
                    #     blurred[:, :, c] = gaussian_filter(hdr_np[:, :, c], sigma=1.0)
                    # hdr_np = blurred


                    filename = os.path.join(render_path, "test_env_j{}_i{}.hdr".format(final_j, final_i))
                    imageio.imwrite(filename, hdr_np, format='HDR')

                    env_light = EnvLight(path=filename, device='cuda', activation="none")
                    ...
                    torchvision.utils.save_image(rendering.clamp(0.0,1.0), os.path.join(render_path, 'illum_{}_j{}_i{}_t{}'.format(render_idx, final_j, final_i, timestep_idx) + ".png"))
                ...
                # Save videos
                vid_name = "render_relight_with_rotating_light{}_view{}_ts{}".format(pipeline.diffuse_sample_num, render_idx, timestep_idx)
...
if __name__ == "__main__":
    ...
    render_set(model.extract(args), pipeline.extract(args), args.load_iter, args.colmap_convention)